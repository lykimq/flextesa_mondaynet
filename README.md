...
## Note
This is the hack version that clone from flextesa to be able to run the SCORU on MondayNet.
The changes can be found in the file `diff.patch`.

The folder `artifacts.zip` is a download from the artifacts build-x86-64 on tezos/master commit.
To get this artifacts you can click on the green check and open the pipeline run, at the build, choose:
`build:static-x86_64` and download this package as `artifacts.zip`.
The one use in this example is downloaded from: https://gitlab.com/tezos/tezos/-/jobs/3399162667. 

To build the flextesa use (decompress `artifacts.zip` as a folder `octez-binaries`): 
```
# remove cache
